...
class Client {
    ...
    // }
		...
		while(myBool) {
			...
			if(in.ready() == true) {
	      int msg = in.read(); //read packet from server

	      // IF GAME PACKET
	      if(msg == 0) {
	        wordLength = in.read();
	        numIncorrect = in.read();
	        numBlanks = 0;
	        ...
	        for (int i = 0; i < numIncorrect; i++) {
						// int j = in.read();
						// char str = getChar(j);
	          incorrectGuesses = incorrectGuesses + getChar(in.read()) + " ";
	        }
	        System.out.println("Incorrect Guesses: " + incorrectGuesses);
	        System.out.println("");
	        ...
	        if (numBlanks != 0 && numIncorrect < 6) {
	          ...
	          //userGuess = checkInput(userGuess, inUser, numIncorrect, wordLength, parts); //check the input given by user
						char ch = getChar(1);
	          out.writeBytes(ch + userGuess); // send user's guess to server
	        }
	        ...
	        //System.out.println(serverMsg);
					serverMsg = "";
					for(int i = 0; i < msg; i++) {
						serverMsg = serverMsg + getChar(in.read());
					}
					System.out.println(serverMsg);
	        if (serverMsg.contains("Game Over!") || serverMsg.contains("server-overload")) {
	          myBool = false;
